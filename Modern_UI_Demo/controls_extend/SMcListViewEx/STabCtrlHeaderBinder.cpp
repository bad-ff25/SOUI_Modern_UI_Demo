...
namespace SOUI
{
	...
	void STabCtrlHeaderBinder::Bind(SWindow * _pWnd, int _idx, DWORD _evtcode)
	{
		...
		if (_pWnd)
		{
			...
			if (m_pTabHost->GetCurSel() == _idx)
				_pWnd->ModifyState(WndState_Disable, 0, TRUE);
		}
	}
	...
	BOOL STabCtrlHeaderBinder::TabSelChanged(EventTabSelChanged *evt_sel)
	{
		...
		while (pos)
		{
			...
			{
				SWindow* select = pair->m_key->FindChildByName(L"select");
			    if (select != nullptr)
				{
				    select->SetVisible(TRUE, FALSE);
				}
				pair->m_key->ModifyState(WndState_Disable,0,TRUE);
				//pair->m_key->SetCheck(TRUE);
				pair->m_key->SetFocus();
			}
			else if (pair->m_value == (int)evt_sel->uOldSel)
			{
                ...
                if (select)
                {
                    select->SetVisible(FALSE, FALSE);
                }
				pair->m_key->ModifyState(0, WndState_Disable, TRUE);
